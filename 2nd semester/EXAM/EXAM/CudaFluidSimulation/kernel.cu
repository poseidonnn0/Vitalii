...
void dR(int d)
{
	...
	{
		if (abs(config.radius - abs(d)) < 5000)
		{
			config.radius += d;
		}
	}
...
__device__ Particle interpolate(Vec2 v, Particle* field, size_t xSize, size_t ySize) // биленейна€ интерпол€ци€
{
	float x1 = (int)v.x;
	float y1 = (int)v.y;
	float x2 = (int)v.x + 1;
...
{// метод якоби дл€ проверки на границы экрана(проверка скорости)
...
{ // јналогичный метод якоби (дл€ координат текстуры)
...
{ // «амена цвета по методу якоби с проверкой на границы
...
{ // —читывание дивергенции скорости
